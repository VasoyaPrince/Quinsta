package com.example.quinsta.crop.preferences
...
internal fun CropPropertySelectionMenu(
    ...
    Title("Crop Type")
    CropTypeDialogSelection(cropType = cropType, onCropTypeChange = { cropTypeChange ->
    ...
    Title("Aspect Ratio")
    AspectRatioSelection(aspectRatio = aspectRatio, onAspectRatioChange = {
        onCropPropertiesChange(
    ...
    Title("Frame")
    CropFrameSelection(aspectRatio = aspectRatio,
        cropFrameFactory = cropFrameFactory,
    ...
    if (cropType == CropType.Dynamic) {
        Title("Handle Size")
        DpSliderSelection(
            value = handleSize, onValueChange = {
                onCropPropertiesChange(
                    cropProperties.copy(handleSize = density.run { it.toPx() })
                )
            }, lowerBound = 10.dp, upperBound = 40.dp
        )
    }
}
...
internal fun CropGestureSelectionMenu(
    cropProperties: CropProperties, onCropPropertiesChange: (CropProperties) -> Unit
) {
    // Gestures
    ...
    Title("Pan Enabled")
    PanEnableSelection(panEnabled = pannable, onPanEnabledChange = {
        onCropPropertiesChange(
    ...
    Title("Fling")
    FlingEnableSelection(flingEnabled = flingEnabled, onFlingEnabledChange = {
        onCropPropertiesChange(
    ...
    Title("Zoom Enabled")
    ZoomEnableSelection(zoomEnabled = zoomable, onZoomEnabledChange = {
        onCropPropertiesChange(
    ...
    AnimatedVisibility(visible = zoomable) {
        Column {
            ...
            MaxZoomSelection(
                maxZoom = maxZoom, onMaxZoomChange = {
...
internal fun AspectRatioSelection(
    aspectRatio: AspectRatio, onAspectRatioChange: (CropAspectRatio) -> Unit
) {
    ...
    AnimatedAspectRatioSelection(
        modifier = Modifier.fillMaxWidth(), initialSelectedIndex = initialSelectedIndex
    ) {
...
internal fun FlingEnableSelection(
    flingEnabled: Boolean, onFlingEnabledChange: (Boolean) -> Unit
) {
    FullRowSwitch(
        label = "Enable fling gesture", state = flingEnabled, onStateChange = onFlingEnabledChange
    )
...
internal fun PanEnableSelection(
    panEnabled: Boolean, onPanEnabledChange: (Boolean) -> Unit
) {
    FullRowSwitch(
        label = "Enable pan gesture", state = panEnabled, onStateChange = onPanEnabledChange
    )
}
...
internal fun ZoomEnableSelection(
    ...
    FullRowSwitch(
        label = "Enable zoom gesture", state = zoomEnabled, onStateChange = onZoomEnabledChange
    )
}

@Composable
internal fun MaxZoomSelection(
    maxZoom: Float, onMaxZoomChange: (Float) -> Unit, valueRange: ClosedFloatingPointRange<Float>
) {
    SliderSelection(
        value = maxZoom, onValueChange = onMaxZoomChange, valueRange = valueRange
    )
}
